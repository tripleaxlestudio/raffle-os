...
export function ProductionDrawPresentation({ result, mode, eventName, eventId = 'production-event', displayConfigurationId = eventId, prizeCategory, prizeName, checkpoints, practiceResult, onFailure, initialPresentation, onHandoff, sharedPublisher, onResetPractice, resetPending = false, presentationPolicy = PRESENTATION_POLICY, presentationConfiguration, audienceStatus, recap, backTo, onRecoveryBack }: ProductionDrawPresentationProps) {
  ...
  if (stage === 'rolling') return <PresentationFrame backTo={backTo} result={result} recap={recap} previewStage="rolling" audienceStatus={audienceStatus} blackoutRequested={blackoutRequested} blackoutControl={blackoutControl} operatorControl={manualRolling ? null : resetControl} mode={mode} eventName={eventName} prizeCategory={prizeCategory} prizeName={prizeName} heading={manualRolling ? 'Rolling — winner locked' : 'Rolling'} announcement={manualRolling ? 'Rolling is active. The winner is already locked. Waiting for Stop and Reveal.' : 'Rolling presentation active'}><div className="production-rolling" aria-label={manualRolling ? 'Rolling is active; winner locked' : 'Rolling presentation active'}><span>• • • • •</span><span>ROLLING</span><span>— — — — —</span></div><p className="presentation-progress">{manualRolling ? 'Waiting for Stop & Reveal' : `${recap?.rollingSeconds ?? Math.round(presentationPolicy.rollingDurationMs / 1000)}s rolling presentation`}</p>{manualRolling ? <div className="production-manual-stop-wrap"><Button variant="danger" size="lg" className="production-manual-stop" onClick={() => { void controller.stopRollingAndReveal() }}>STOP &amp; REVEAL</Button></div> : <Button onClick={() => { void controller.skip() }}>Skip animation</Button>}</PresentationFrame>
