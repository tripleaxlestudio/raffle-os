...
function expectFullViewportAudienceStage() {
  ...
  expect(Array.from(page?.children ?? []).filter((child) => !child.classList.contains('audience-fullscreen-controls') && !child.classList.contains('sr-only'))).toHaveLength(1)
}
...
describe('production Audience route', () => {
  ...
  it('renders valid public snapshots without operator controls', () => {
    ...
    expect(screen.getByLabelText('Countdown: 2')).toBeVisible()
    expectFullViewportAudienceStage()
    ...
    expect(screen.getByText('3 Winner')).toBeVisible()
  ...
  it('renders an authoritative ready snapshot as the next draw without claiming current execution', () => {
    ...
    expect(screen.getByText('Door Prize')).toBeVisible()
    expect(screen.getByText('10 Winner')).toBeVisible()
  ...
  it('updates only the authoritative confirmed tile in place during partial verification', () => {
    ...
    expect(after.map((tile) => tile.textContent?.replace('Dikonfirmasi', '').replace('Tertunda', '').trim())).toEqual(['00073', '00052', '00059'])
    controller.close()
    publisher.close()
  })
