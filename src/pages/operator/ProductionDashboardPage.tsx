...
export function ProductionDashboardPage() {
  ...
  if (workspace.status === 'empty') return <section aria-labelledby="dashboard-title"><PageHeader eyebrow="Production workspace" headingId="dashboard-title" title="Dashboard" description="No active Event is selected." /><StatusBanner badge="Setup required" title="Select an Event before operating" tone="warning">No fictional Event or readiness values are shown. Event creation and selection will be added in the next product slice.</StatusBanner><p><ButtonLink to="/participants">Open Participants</ButtonLink></p></section>
  if (workspace.status === 'invalid-reference') return <section aria-labelledby="dashboard-title"><PageHeader eyebrow="Production workspace" headingId="dashboard-title" title="Dashboard" description="The saved active Event reference is invalid." /><StatusBanner badge="Recovery required" title="The selected Event could not be found" tone="warning">Stored Event reference {workspace.eventId} does not resolve to an Event. No operational data was inferred.</StatusBanner></section>
  ...
  const primary = workspace.unresolvedSession === null
    ? <ButtonLink size="lg" to="/draw/setup">Open Draw Setup</ButtonLink>
    : <ButtonLink size="lg" to={`/draw/pending/${workspace.unresolvedSession.id}`}>Resume pending result</ButtonLink>
