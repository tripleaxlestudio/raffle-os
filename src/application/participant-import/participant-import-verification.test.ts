...
describe('persisted Participant verification read boundary', () => {
  it('loads the complete ordered dataset and total count without exposing the repository', async () => {
    ...
    await expect(getPersistedParticipantsForEvent(participants, eventId)).resolves.toEqual({ totalCount: 2, records })
    expect(participants.findByEventId).toHaveBeenCalledWith(eventId, { limit: 2, offset: 0 })
