...
describe('production Settings display-test integration', () => {
  ...
  it('keeps one publisher alive across five realtime start/stop cycles', () => {
    ...
    expect(stateEnvelopes.map((envelope) => envelope.sequence)).toEqual(Array.from({ length: 11 }, (_, index) => index + 1))
  ...
  it('delivers the production snapshot through the BroadcastChannel adapter', () => {
    ...
    publisher.start({ drawSessionId: session, stage: 'standby', blackoutRequested: false, displayTest: false, eventName: 'Native Event', eventSubtitle: 'Native subtitle', safeAreaMargin: 48 })
