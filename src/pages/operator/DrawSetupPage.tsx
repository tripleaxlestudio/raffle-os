...
function Confirmation({ mode, view, onCancel, onConfirm, busy }: { mode: AppMode; view: DrawSetupReadyViewModel; onCancel: () => void; onConfirm: () => void; busy: boolean }) {
  return <ConfirmationDialog confirmLabel={busy ? 'Starting…' : mode === 'live' ? 'Confirm live draw' : 'Run practice'} consequence={mode === 'live' ? <>Event <strong>{view.event.name}</strong>, category <strong>{view.category.name}</strong>, and <strong>{view.configuration.requestedWinners}</strong> winner slots will use the frozen eligibility snapshot and record an official pending draw.</> : <>This is a rehearsal only. No official WinnerRecords, DrawSession mutation, or AuditRecord will be written.</>} onCancel={onCancel} onConfirm={onConfirm} open title={mode === 'live' ? 'Confirm live draw start' : 'Run practice rehearsal'} />
}
...
export function DrawSetupPage({ services: suppliedServices }: { services?: DrawSetupProductionServices } = {}) {
  ...
  if (view.state === 'query-failure') return <section className="draw-setup"><PageHeader eyebrow="Draw configuration" headingId="draw-setup-title" title="Draw Setup" description="Readiness unavailable" /><StatusBanner badge="Recoverable error" title="Draw Setup could not be loaded" tone="warning">{view.error.message} Retry after checking the local database.</StatusBanner><Button onClick={() => void load()}>Retry</Button></section>
