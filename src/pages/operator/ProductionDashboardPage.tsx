...
export function ProductionDashboardPage() {
  ...
  const actions = <div className={ui('production-dashboard__secondary-actions')}><ButtonLink icon={<Icon name="Upload" />} variant="secondary" to="/participants">Impor Peserta</ButtonLink><ButtonLink icon={<Icon name="SlidersHorizontal" />} variant="secondary" to="/draw/setup">Pengaturan Undian</ButtonLink></div>
  return <section aria-labelledby="dashboard-title" className={ui('dashboard')}>
    <PageHeader description={`Status Acara: ${workspace.event.status}${workspace.unresolvedSession === null ? ' · Siap untuk undian berikutnya' : ' · Sesi Live perlu perhatian'}`} eyebrow="Ruang kerja produksi" headingId="dashboard-title" title={workspace.event.name} />
    ...
    <section aria-label="Metrik Acara" className={ui('dashboard-metrics dashboard-metrics--production')}>
    ...
    <section className={ui('production-dashboard__preparation')} aria-labelledby="dashboard-preparation-title">
      <div><h2 id="dashboard-preparation-title">Persiapan Acara</h2><p>Periksa data peserta dan konfigurasi sebelum menjalankan undian.</p></div>
      {actions}
    </section>
  </section>
}
...
function DashboardOperations({ audienceStatus, displayUrl, drawState, pendingCount }: { readonly audienceStatus: DisplayConnectionStatus; readonly displayUrl: string | null; readonly drawState: { status: 'loading' } | { status: 'error' } | { status: 'ready'; item: DrawSessionQueueItem | null }; readonly pendingCount: number }) {
  ...
  return <section aria-label="Operasi Dasbor" className={ui('production-dashboard__operations')}>
    <Card className={ui('production-dashboard__next-draw')} padding="md" tone="raised"><div className={ui('production-dashboard__panel-heading')}><div><h2>Undian berikutnya</h2></div><span className={ui('production-dashboard__panel-state')}>{drawState.status === 'loading' ? 'MEMBACA' : item === null ? 'TIDAK ADA UNDIAN SIAP' : 'SIAP'}</span></div>{item === null ? <p className={ui('production-dashboard__empty-copy')}>{drawState.status === 'loading' ? 'Membaca sesi undian siap berikutnya…' : drawState.status === 'error' ? 'Sesi undian siap berikutnya tidak dapat dibaca.' : 'Buat atau selesaikan undian di Pengaturan Undian untuk menyiapkan presentasi berikutnya.'}</p> : <><div className={ui('production-dashboard__next-draw-identity')}><h3>{item.category?.prizeName ?? 'Hadiah tidak tersedia'}</h3><p>{item.category?.name ?? 'Kategori hadiah tidak tersedia'} · {item.winnerCount} pemenang</p></div><p className={ui('production-dashboard__next-draw-state')}>{item.session.mode === 'live' ? 'Live' : 'Latihan'} · Siap dimulai</p></>}<ButtonLink icon={<Icon name="Play" />} size="lg" to="/draw/live">{item === null ? 'Buka Undian' : item.session.mode === 'live' ? 'Mulai Undian' : 'Mulai Latihan'}</ButtonLink></Card>
    <Card className={ui('production-dashboard__operations-panel')} padding="md" tone="raised"><div className={ui('production-dashboard__panel-heading')}><div><h2>Kontrol acara</h2></div></div><dl className={ui('production-dashboard__operation-list')}><div><dt>Tampilan Audiens</dt><dd><AudienceConnectionStatus state={audienceStatus} /></dd></div><div><dt>Menunggu Konfirmasi</dt><dd>{pendingCount}</dd></div></dl><div className={ui('production-dashboard__operation-actions')}>{displayUrl === null ? <ButtonLink icon={<Icon name="MonitorCog" />} variant="secondary" to="/settings">Buka Pengaturan Tampilan</ButtonLink> : <AudienceDisplayButton displayUrl={displayUrl} variant="secondary" />}{pendingCount > 0 ? <ButtonLink icon={<Icon name="ClipboardCheck" />} variant="secondary" to="/draw/pending">Buka Hasil</ButtonLink> : null}</div></Card>
  </section>
}
