  ...
  const progressStep = state.status === 'idle' || state.status === 'reading' || state.status === 'unsupported' || state.status === 'parse-error'
    ...
    : state.status === 'mapping'
      ...
      : state.status === 'validation-ready' || state.status === 'commit-failure' || staged === null || staged.validation.summary.validRows === 0 || hasBlockingDiagnostics || strategy === null
        ? 'validation'
        : 'summary'
  if (!eventLoading && event === null && (eventError?.includes('Tidak ada Acara aktif') === true || eventError?.includes('Acara aktif tidak dapat ditemukan') === true)) return <section className={ui('participant-import production-import-preview')} aria-labelledby="production-import-title"><header className={`${ui('page-header')} production-import-preview__header`}><div className={ui('page-header__copy')}><p className={ui('page-header__eyebrow')}>Operasi Peserta</p><h1 id="production-import-title">Impor Peserta</h1><p className={ui('page-header__description')}>Impor Peserta ke Acara aktif.</p></div></header><ProductionSetupRequired title="Acara diperlukan untuk impor Peserta" description="Pilih atau buat Acara sebelum mengimpor Peserta." /></section>
  return <section className={ui('participant-import production-import-preview')} aria-label="Impor file Peserta" data-workflow-state={state.status}>
    <header className={`${ui('page-header')} production-import-preview__header`}><div className={ui('page-header__copy')}><p className={ui('page-header__eyebrow')}>Operasi Peserta</p><div className={ui('production-import-preview__title-row')}><h1 id="production-import-title">Impor Peserta</h1></div><p className={ui('page-header__description')}>Siapkan, validasi, dan impor Peserta secara atomik ke Acara aktif.</p></div></header>
    ...
    {showPreparation ? <div className={ui('production-import-preview__workspace')} aria-label="Ruang kerja validasi">
      <div className={ui('production-import-preview__main')}>
        {showPreparation ? <>
        ...
        {state.status === 'reading' || state.status === 'committing' ? <p className={ui('production-import-preview__progress')} role="status" aria-live="polite">{state.status === 'reading' ? `Membaca ${state.fileName}…` : 'Menyimpan impor Peserta secara atomik…'}</p> : null}
        {state.status === 'unsupported' || state.status === 'parse-error' || state.status === 'commit-failure' ? <div className={ui('production-import-preview__error')} role="alert"><StatusBanner badge="Perlu tindakan" title={state.status === 'commit-failure' ? 'Impor gagal dengan aman' : state.status === 'unsupported' ? 'File tidak didukung' : 'File tidak dapat dibaca'} tone="warning">{state.message}</StatusBanner>{state.status === 'commit-failure' ? <Button icon={<Icon name="RefreshCw" />} variant="secondary" onClick={() => setState({ status: 'validation-ready', staged: state.staged, strategy: state.strategy })}>Tinjau ulang</Button> : null}</div> : null}
      ...
      <aside className={ui('production-import-preview__side')}>
        {showPreparation && staged ? <><ValidationPanel validation={staged.validation} /><div className={ui('production-import-preview__final-workflow')}><StrategyPanel strategy={strategy} disabled={state.status === 'committing' || state.status === 'confirmation' || !mutable} onChoose={chooseStrategy} /><Card className={ui('production-import-preview__action-card')} padding="md"><div><p className={ui('production-import-preview__action-kicker')}>Siap untuk dilanjutkan</p><h2>Konfirmasi impor</h2><p>Tinjau batch tervalidasi dan strategi yang dipilih sebelum membuka konfirmasi akhir.</p></div><Button icon={<Icon name="ClipboardCheck" />} disabled={!ready || state.status === 'committing' || state.status === 'confirmation'} isLoading={state.status === 'committing'} onClick={() => { if ('staged' in state && state.strategy) { setState({ status: 'confirmation', staged: state.staged, strategy: state.strategy }); setConfirmOpen(true) } }}>Tinjau dan konfirmasi impor</Button></Card></div></> : null}
        <div className={ui('production-import-preview__persisted')}><PersistedParticipantSection event={event} verification={verification} /></div>
      </aside>
    </div> : <div className={ui('production-import-preview__success-layout')} aria-label="Impor Peserta selesai">
      {state.status === 'commit-success' ? <div className={ui('production-import-preview__success-state')}><SuccessPanel event={event} state={state} /><Button icon={<Icon name="Upload" />} variant="secondary" size="sm" onClick={resetStagedImport}>Impor File Lain</Button></div> : null}
    ...
    <Modal open={confirmOpen && strategy === 'replace'} title="Konfirmasi impor Ganti" onClose={() => { setConfirmOpen(false); if ('staged' in state && state.strategy) setState({ status: 'strategy-selection', staged: state.staged, strategy: state.strategy }) }} footer={<><Button icon={<Icon name="X" />} variant="secondary" onClick={() => setConfirmOpen(false)}>Batal</Button><Button icon={<Icon name="CircleCheck" />} disabled={!replaceAcknowledged} onClick={() => void commit()}>Konfirmasi Ganti</Button></>}><ConfirmationContents event={event} staged={staged} strategy="replace" currentParticipantCount={currentParticipantCount} /><label className={ui('import-acknowledgement')}><input type="checkbox" checked={replaceAcknowledged} onChange={(e) => setReplaceAcknowledged(e.target.checked)} /> Saya memahami bahwa Peserta saat ini dalam Acara akan dihapus dan diganti secara atomik.</label></Modal>
  </section>
}

function EventPanel({ event, loading, error }: { event: Event | null; loading: boolean; error: string | null }) {
  const ui = useUiClass()
 return <Card className={ui('production-import-preview__summary-card')} tone="raised" aria-labelledby="selected-event-title"><div className={ui('production-import-preview__card-heading')}><div><p className={ui('production-import-preview__card-kicker')}>Target impor</p><h2 id="selected-event-title">Acara yang Dipilih</h2></div>{event ? <Badge variant={event.status === 'draft' ? 'success' : 'danger'}>{productionDomainLabel(event.status)}</Badge> : <Badge variant="warning">Diblokir</Badge>}</div>{loading ? <p role="status">Mencari Acara aktif…</p> : error ? <p role="alert">{error}</p> : event ? <><p className={ui('production-import-preview__event-name')}>{event.name}</p><p className={ui('production-import-preview__mutability')}>{event.status !== 'draft' ? 'Acara ini tidak mengizinkan impor Peserta karena tidak dapat diubah.' : 'Acara draf · target impor dapat diubah.'}</p></> : null}</Card> }

function UploadPanel({ inputRef, fileInfo, committing, onFile, onRemove }: { inputRef: RefObject<HTMLInputElement | null>; fileInfo: { name: string; type: string; size: number } | null; committing: boolean; onFile: (file: File | undefined) => void; onRemove: () => void }) {
  const ui = useUiClass()
 return <Card className={ui('production-import-preview__upload')} tone="raised" aria-labelledby="upload-title"><div className={ui('production-import-preview__card-heading')}><div><p className={ui('production-import-preview__card-kicker')}>Langkah 1</p><h2 id="upload-title">Unggah file Peserta</h2></div><div className={ui('production-import-preview__badges')}><Badge variant="info">CSV</Badge><Badge variant="info">XLSX</Badge></div></div><div className={ui('production-import-preview__file-picker')}><span className={ui(`production-import-preview__file-name${fileInfo ? '' : ' is-empty'}`)} title={fileInfo?.name}>{fileInfo?.name ?? 'Belum ada file dipilih'}</span><input aria-label="File Peserta" ref={inputRef} id="participant-file" className={ui('production-import-preview__file-input')} type="file" accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" disabled={committing} onChange={(e) => onFile(e.currentTarget.files?.[0])} /><Button icon={<Icon name="FolderOpen" />} variant="secondary" size="sm" onClick={() => inputRef.current?.click()} disabled={committing}>Pilih file</Button></div><p className={ui('production-import-preview__hint')}>CSV atau XLSX · gunakan file di bawah 10 MB agar pratinjau lancar.</p>{fileInfo ? <div className={ui('production-import-preview__file-meta')} role="status"><div><strong>{fileInfo.name}</strong><span>{fileInfo.type} · {formatBytes(fileInfo.size)}</span></div><Button icon={<Icon name="FileX" />} variant="quiet" size="sm" onClick={onRemove} disabled={committing}>Hapus file</Button></div> : null}</Card> }

function MappingAndPreview({ staged, disabled, onMappingChange, onWorksheetChange }: { staged: StagedImport; disabled: boolean; onMappingChange: (field: ColumnMapping['targetField'], value: string | null) => void; onWorksheetChange: (name: string) => void }) {
  const ui = useUiClass()
 const xlsx = staged.parsed.format === 'xlsx' ? staged.parsed : null; return <Card className={ui('production-import-preview__mapping')} tone="raised" aria-labelledby="mapping-title"><div className={ui('production-import-preview__card-heading')}><div><p className={ui('production-import-preview__card-kicker')}>Langkah 2</p><h2 id="mapping-title">Petakan kolom sumber</h2></div></div><p className={ui('production-import-preview__hint')}>Petakan setiap kolom sumber ke field Peserta. Perubahan pemetaan menghapus konfirmasi dan status berhasil sebelumnya.</p><div className={ui('production-import-preview__mapping-grid')}>{PARTICIPANT_IMPORT_FIELDS.map((field) => { const mapping = staged.mappings.find((item) => item.targetField === field.field); return <Select key={field.field} id={`mapping-${field.field}`} label={`${participantFieldLabel(field.field)} · ${field.requirement === 'required' ? 'Wajib' : 'Opsional'}`} description={field.field === 'ticketNumber' ? 'Nilai tiket tetap berupa string persis.' : undefined} disabled={disabled} value={mapping?.sourceColumn ?? ''} onChange={(e) => onMappingChange(field.field, e.target.value || null)}><option value="">Tidak dipetakan</option>{staged.parsed.headers.filter(Boolean).map((header) => <option key={header} value={header}>{header}</option>)}</Select> })}{xlsx && xlsx.worksheets.filter((sheet) => sheet.visibility === 'visible').length > 1 ? <Select id="xlsx-worksheet" label="Lembar kerja" disabled={disabled} value={xlsx.sheetName} onChange={(e) => onWorksheetChange(e.target.value)}>{xlsx.worksheets.filter((sheet) => sheet.visibility === 'visible').map((sheet) => <option key={sheet.name} value={sheet.name}>{sheet.name}</option>)}</Select> : null}</div></Card> }
...
function PaginatedPreviewTable({ parsed }: { parsed: ParsedImportFile }) {
  ...
  return <Card className={ui('production-import-preview__table-card')} tone="raised" aria-labelledby="raw-preview-title"><div className={ui('production-import-preview__card-heading')}><div><p className={ui('production-import-preview__card-kicker')}>Data sumber</p><h2 id="raw-preview-title">Pratinjau baris mentah</h2><p className={ui('production-import-preview__hint')}>{formatProductionNumber(parsed.rows.length)} baris. Nilai ditampilkan persis seperti saat dibaca.</p></div></div><Table caption="Baris Peserta hasil pembacaan"><thead><tr><TableHeader>Baris sumber</TableHeader>{parsed.headers.map((header) => <TableHeader key={header}>{header}</TableHeader>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.rowNumber}><th scope="row">{row.rowNumber}</th>{parsed.headers.map((header) => <td key={header} className={header.toLowerCase().includes('ticket') ? ui('production-import-preview__ticket') : undefined}>{typeof row.values[header] === 'string' ? row.values[header] : String(row.values[header] ?? '')}</td>)}</tr>)}</tbody></Table><Pagination label="Navigasi halaman baris mentah" page={safePage} pageSize={pageSize} totalItems={parsed.rows.length} onPageChange={setPage} onPageSizeChange={(size) => { setPageSize(size); setPage(1) }} /></Card>
}
export function PreviewTable({ parsed }: { parsed: ParsedImportFile }) {
  const ui = useUiClass()
 const rows = parsed.rows.slice(0, MAX_PREVIEW_ROWS); return <Card className={ui('production-import-preview__table-card')} tone="raised" aria-labelledby="raw-preview-title"><div className={ui('production-import-preview__card-heading')}><div><p className={ui('production-import-preview__card-kicker')}>Data sumber</p><h2 id="raw-preview-title">Pratinjau baris mentah</h2><p className={ui('production-import-preview__hint')}>Menampilkan {rows.length} dari {parsed.rows.length} baris. Nilai ditampilkan persis seperti saat dibaca.</p></div></div>{rows.length < parsed.rows.length ? <p className={ui('production-import-preview__truncation')}>Pratinjau dibatasi hingga {MAX_PREVIEW_ROWS} baris.</p> : null}<Table caption="Baris Peserta hasil pembacaan"><thead><tr><TableHeader>Baris sumber</TableHeader>{parsed.headers.map((header) => <TableHeader key={header}>{header}</TableHeader>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.rowNumber}><th scope="row">{row.rowNumber}</th>{parsed.headers.map((header) => <td key={header} className={header.toLowerCase().includes('ticket') ? ui('production-import-preview__ticket') : undefined}>{typeof row.values[header] === 'string' ? row.values[header] : String(row.values[header] ?? '')}</td>)}</tr>)}</tbody></Table></Card> }
function ValidationPanel({ validation }: { validation: ParticipantImportValidationResult }) {
  const ui = useUiClass()
 const issues = validation.rows.flatMap((row) => row.issues).slice(0, MAX_ISSUE_ROWS_SHOWN); return <Card className={ui('production-import-preview__validation')} tone="raised" aria-labelledby="validation-title"><div className={ui('production-import-preview__card-heading')}><div><p className={ui('production-import-preview__card-kicker')}>Langkah 3</p><h2 id="validation-title">Diagnostik dan ringkasan validasi</h2></div><Badge variant={validation.summary.invalidRows === 0 ? 'success' : 'warning'}>{validation.summary.invalidRows === 0 ? 'Siap' : 'Tinjau masalah'}</Badge></div><div className={ui('production-import-preview__metrics')} aria-label="Metrik validasi"><Metric label="Total baris" value={validation.summary.totalRows} /><Metric label="Draf valid" value={validation.summary.validRows} /><Metric label="Baris tidak valid" value={validation.summary.invalidRows} /><Metric label="Duplikat" value={validation.summary.duplicateRows} /><Metric label="Masalah pemblokir" value={validation.summary.issueCount} /></div><p className={ui('production-import-preview__metric-summary')}>Total baris: {validation.summary.totalRows} · Draf valid: {validation.summary.validRows} · Baris tidak valid: {validation.summary.invalidRows} · Baris duplikat: {validation.summary.duplicateRows}</p><div className={ui('production-import-preview__issue-list')}><p>Diagnostik baris</p><Table caption="Validasi per baris sumber"><thead><tr><TableHeader>Baris</TableHeader><TableHeader>Nomor Tiket</TableHeader><TableHeader>Hasil</TableHeader></tr></thead><tbody>{validation.rows.slice(0, MAX_PREVIEW_ROWS).map((row) => <tr key={row.normalized.rowNumber}><th scope="row">{row.normalized.rowNumber}</th><td className={ui('production-import-preview__ticket')}>{row.normalized.ticketNumber ?? '—'}</td><td>{row.participantDraft ? 'Draf valid' : 'Tidak valid — tidak dikirim ke penyimpanan'}</td></tr>)}</tbody></Table>{issues.length > 0 ? <ul>{issues.map((issue) => <li key={`${issue.rowNumber}-${issue.code}`}>Baris {issue.rowNumber}: {issue.message}</li>)}</ul> : <p>Tidak ditemukan masalah baris.</p>}{validation.summary.issueCount > MAX_ISSUE_ROWS_SHOWN ? <p>Hanya {MAX_ISSUE_ROWS_SHOWN} masalah pertama yang ditampilkan.</p> : null}</div><p className={ui('production-import-preview__safe-note')}>Belum ada data Peserta yang disimpan.</p></Card> }
...
function PersistedParticipantSection({ event, verification }: { event: Event | null; verification: VerificationState }) {
  ...
  const checkedInCount = verification.status === 'ready'
    ...
    : null
  return <Card className={ui('production-import-preview__summary-card')} tone="raised" aria-labelledby="persisted-participants-title">
    ...
    {verification.status === 'ready' ? <>
      ...
      <Table caption="Peserta tersimpan"><thead><tr><TableHeader>Nomor Tiket</TableHeader><TableHeader>Nama</TableHeader><TableHeader>Grup</TableHeader><TableHeader>Check-in</TableHeader></tr></thead><tbody>{visibleRecords.map((record) => <tr key={record.id}><th scope="row" className={ui('production-import-preview__ticket')}><code>{record.ticketNumber}</code></th><td>{record.name ?? '—'}</td><td>{record.group ?? '—'}</td><td>{record.isCheckedIn ? 'Sudah check-in' : 'Belum check-in'}</td></tr>)}</tbody></Table>
...
function SuccessPanel({ event, state }: { event: Event | null; state: Extract<ProductionState, { status: 'commit-success' }> }) {
  const ui = useUiClass()
 return <Card className={ui('production-import-preview__success')} tone="raised" role="status" aria-live="polite"><StatusBanner badge="Selesai" title="Impor Peserta selesai" tone="success">Disimpan ke {event?.name ?? 'Acara yang dipilih'} dengan strategi {state.strategy === 'replace' ? 'Ganti' : 'Gabung'}.</StatusBanner><div className={ui('production-import-preview__success-metrics')}><Metric label="Dimasukkan" value={state.result.insertedCount} /><Metric label="Dihapus / diganti" value={state.result.removedCount} /><Metric label="Tidak berubah" value={state.result.unchangedCount} /></div><p className={ui('production-import-preview__metric-summary')}>Dimasukkan: {formatProductionNumber(state.result.insertedCount)} · Dihapus/diganti: {formatProductionNumber(state.result.removedCount)} · Tidak berubah: {formatProductionNumber(state.result.unchangedCount)}</p><p>Selesai: {state.result.completedAt} · Operasi: {state.result.operationId}</p><Button icon={<Icon name="CircleCheck" />} onClick={() => window.location.reload()}>Mulai Impor Baru</Button></Card> }
