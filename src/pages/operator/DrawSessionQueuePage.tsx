...
export function DrawControlDeck({ connection, deck, displayUrl, selectedMode, setSelectedMode }: { readonly connection: ReturnType<typeof presentAudienceConnection>; readonly deck: DrawSessionQueueDeck; readonly displayUrl: string | null; readonly selectedMode: 'practice' | 'live'; readonly setSelectedMode: (mode: 'practice' | 'live') => void }) {
  ...
  return <Card className={`draw-control-deck draw-control-deck--${liveSelected ? 'live' : 'practice'}`} padding="none">
    ...
    <div className="draw-control-deck__body">
      ...
      <div className="draw-control-deck__audience"><div className="draw-control-deck__audience-heading"><div><span>Audience Display</span><strong><span aria-hidden="true" className={`draw-control-deck__connection-dot draw-control-deck__connection-dot--${connection.tone}`} />{connection.label}</strong><p>{connection.acknowledged ? formatAcknowledgedAt(connection.acknowledgedAt) : connection.detail}</p></div></div><div className="draw-control-deck__audience-actions">{displayUrl === null ? <Button disabled variant="secondary">Open Audience Display</Button> : <ButtonLink to={displayUrl} target="_blank" rel="noreferrer" variant="secondary">Open Audience Display</ButtonLink>}</div></div>
      {audienceWarning}
      <div className="draw-control-deck__actions">
        <section className="draw-control-deck__action-panel draw-control-deck__action-panel--practice" aria-labelledby={`practice-action-${deck.key}`}>
          ...
          <div className="draw-control-deck__action-panel-button">{canAct ? <ButtonLink size="lg" variant={liveSelected ? 'danger' : 'primary'} to={selectedAction.to}>{liveSelected && selectedAction.kind === 'run' ? 'Start Live Draw' : selectedPresentation?.actionLabel}</ButtonLink> : null}{selectedAction !== null && selectedPresentation?.historical ? <ButtonLink to={`/history/${selected?.session.id}`} variant="quiet">View session details</ButtonLink> : null}</div>
        </section>
