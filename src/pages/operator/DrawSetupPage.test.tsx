...
describe('Draw Setup persisted authoring', () => {
  ...
  it('shows authoritative capacity diagnostics and readiness', async () => {
    ...
    const primaryMetrics = screen.getAllByText('Eligible pool').find((element) => element.tagName === 'DT')?.closest('dl')
    expect(primaryMetrics).toContainElement(screen.getByText('Requested winners'))
    expect(primaryMetrics?.querySelectorAll('dt')).toHaveLength(2)
    expect(screen.getByText('Total participants').closest('dl')?.querySelectorAll('dt')).toHaveLength(3)
