...
import { Badge, Button, ButtonLink, Card, Icon, Input, Select, Table, type IconName } from '../../shared/ui/index.ts'
...
export function AuditTimeline({ reconstruction }: { readonly reconstruction: HistoryReconstruction }) {
  ...
  return <Card className="history-audit-timeline" padding="none" tone="raised"><div className="history-detail__audit-heading"><h2>Linimasa Audit</h2><p>Bukti kronologis tersimpan untuk sesi Live ini.</p></div>{entries.length === 0 ? <p>Tidak ada entri audit tersimpan yang terkait dengan sesi ini.</p> : <ol>{entries.map((entry) => <li key={entry.id}><time dateTime={entry.timestamp}>{timestamp(entry.timestamp)}</time><div className="history-audit-timeline__event"><strong>{entry.action}</strong><span>{entry.actor ?? 'Aktor tidak tersedia'}</span></div><div className="history-audit-timeline__supporting">{entry.ticketNumber ? <code className="result-ticket">{entry.ticketNumber}</code> : null}{entry.originalTicketNumber && entry.replacementTicketNumber ? <span className="history-audit-timeline__replacement"><code>{entry.originalTicketNumber}</code> → <code>{entry.replacementTicketNumber}</code></span> : null}{entry.reason ? <span>Alasan: {label(entry.reason)}</span> : null}{entry.note ? <span>Catatan: {entry.note}</span> : null}</div></li>)}</ol>}</Card>
}
...
function WinnerRow({ winner, lineage }: { readonly winner: ReconstructedWinner; readonly lineage: ReconstructedHistorySession['lineages'][number] | undefined }) {
  return <tr><td className="history-detail__sequence">#{winner.sequence}</td><td className="history-detail__ticket"><code className="result-ticket">{winner.ticketNumber}</code></td><td className="history-detail__winner-status"><Badge variant={winnerVariant(winner.status)}><Icon name={winnerStatusIcon(winner.status)} size={16} />{label(winner.status)}</Badge></td><td className="history-detail__timestamp">{timestamp(winner.confirmationTimestamp)}</td><td className="history-detail__timestamp">{timestamp(winner.cancellationTimestamp)}</td><td className="history-detail__replacement">{lineage === undefined ? <span className="history-detail__replacement-empty">Pilihan asli dipertahankan</span> : <span className="history-detail__replacement-context"><span className="history-detail__replacement-flow"><code>{lineage.originalTicketNumber}</code><Icon name="ArrowRight" size={16} /><code>{lineage.replacementTicketNumber}</code></span><small>{label(lineage.reason)}{lineage.reasonNote ? ` · ${lineage.reasonNote}` : ''}</small></span>}</td></tr>
}
