...
function configuration(overrides: Partial<DrawPresentationConfiguration> = {}): DrawPresentationConfiguration { return { presentationMode: 'random-number-roll', rollStopMode: 'manual', rollDurationSeconds: 5, rollSpeedPerSecond: 20, revealMode: 'sequential', ...overrides } }
...
describe('PresentationController', () => {
  ...
  it('persists countdown, rolls, and reveals with one immutable result', async () => {
    ...
    const states: string[] = []
    const controller = new PresentationController({ result, mode: 'live', presentationConfiguration: configuration(), clock: makeClock(), persistStage: async (stage) => { persisted.push(stage) }, onState: (state) => { states.push(state.stage) } })
  ...
  it('publishes each visible countdown value once before rolling', async () => {
    ...
    const controller = new PresentationController({ result, mode: 'practice', presentationConfiguration: configuration(), clock: makeClock(), persistStage: async () => undefined, onState: (state) => states.push({ stage: state.stage, countdownLabel: state.countdownLabel }) })
    await controller.start()
    await vi.advanceTimersByTimeAsync(1000)
    await vi.advanceTimersByTimeAsync(1000)
    await vi.advanceTimersByTimeAsync(1000)
    expect(states.filter((state) => state.stage === 'countdown').map((state) => state.countdownLabel)).toEqual([3, 2, 1])
    expect(states.filter((state) => state.stage === 'rolling')).toHaveLength(1)
  })

  it('keeps countdown 1 published until the rolling checkpoint is persisted', async () => {
    ...
    const states: Array<{ stage: string; countdownLabel: number | null }> = []
    const persistStage = vi.fn(async (stage: string) => {
      if (stage === 'rolling') await rollingPersistence
    })
    const controller = new PresentationController({ result, mode: 'live', presentationConfiguration: configuration(), clock: makeClock(), persistStage, onState: (state) => states.push({ stage: state.stage, countdownLabel: state.countdownLabel }) })

    await controller.start()
    await vi.advanceTimersByTimeAsync(2000)
    expect(controller.getState()).toMatchObject({ stage: 'countdown', countdownLabel: 1 })

    vi.advanceTimersByTime(1000)
    await Promise.resolve()

    expect(persistStage.mock.calls.map(([stage]) => stage)).toEqual(['countdown', 'rolling'])
    expect(controller.getState()).toMatchObject({ stage: 'countdown', countdownLabel: 1 })
    expect(states.filter((state) => state.stage === 'countdown').map((state) => state.countdownLabel)).toEqual([3, 2, 1])
    expect(states.filter((state) => state.stage === 'rolling')).toHaveLength(0)

    resolveRollingPersistence?.()
    await Promise.resolve()
    await Promise.resolve()

    expect(controller.getState()).toMatchObject({ stage: 'rolling', countdownLabel: null })
    expect(states.filter((state) => state.stage === 'rolling')).toHaveLength(1)
  })
  ...
  it.each([5, 8, 12] as const)('ignores a persisted timed %s-second duration and waits for manual Stop', async (seconds) => {
    const persisted: string[] = []
    const persistStage = vi.fn(async (stage: string) => { persisted.push(stage) })
    const controller = new PresentationController({ result, mode: 'practice', presentationConfiguration: configuration({ rollStopMode: 'timed', rollDurationSeconds: seconds }), clock: makeClock(), persistStage, onState: () => undefined })
    await controller.start()
    await vi.advanceTimersByTimeAsync(3000)
    expect(controller.getState().stage).toBe('rolling')
    await vi.advanceTimersByTimeAsync(seconds * 1000 + 30000)
    expect(controller.getState().stage).toBe('rolling')
    expect(controller.getState().presentationConfiguration.rollStopMode).toBe('manual')
    expect(controller.getState().presentationConfiguration.rollSpeedPerSecond).toBe(12)
    expect(controller.getState().presentationConfiguration.revealMode).toBe('all-together')
    await controller.stopRollingAndReveal()
    expect(controller.getState().stage).toBe('reveal')
    expect(persisted).toEqual(['countdown', 'rolling', 'reveal'])
    expect(persistStage).toHaveBeenCalledTimes(3)
    expect(controller.result).toBe(result)
  })
