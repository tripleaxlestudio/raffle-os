...
describe('production Audio settings UI contract', () => {
  it('keeps the authoritative audio contract in a compact two-column layout', () => {
    ...
    expect(source).toContain('className="ui-toggle__input"')
    expect(source).toContain('className="ui-toggle__track"')
    expect(source).toContain('className="ui-toggle__thumb"')
    expect(source).toContain('aria-checked={settings.audioEnabled}')
    expect(source).toContain('settings-audio-enabled-toggle')
    expect(source).toContain('settings-audio-enabled-toggle__control')
    expect(source).toContain('settings-volume-block')
    expect(source).toContain('settings-volume-block__header')
    expect(source).toContain("settings-volume-block ${!settings.audioEnabled ? 'is-disabled' : ''}")
    expect(source).toContain("settings.audioEnabled ? 'Enabled' : 'Disabled'")
