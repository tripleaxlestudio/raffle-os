...
import { validateBlackoutAppearance, validateDisplayAppearance, validateSafeAreaMargin, validateTargetResolution } from '../../domain/display/display-configuration.types.ts'
...
export type DisplayConfigurationInput = Readonly<Pick<DisplayConfiguration, 'targetResolution' | 'safeAreaMargin' | 'blackoutAppearance'> & { readonly appearance?: DisplayConfiguration['appearance'] }>
...
export function createDisplayConfigurationService(dependencies: {
  ...
  return {
    ...
    async saveForEvent(event, input, existing) {
      ...
      const appearance = input.appearance === undefined ? undefined : validateDisplayAppearance(input.appearance)
      if (appearance !== undefined && !appearance.ok) throw new Error(appearance.error.message)
      const now = timestamp()
      const configuration: DisplayConfiguration = existing === null || existing === undefined
        ? { id: createDisplayConfigurationId(), eventId: event.id, targetResolution: resolution.value, safeAreaMargin: margin.value, blackoutAppearance: blackout.value, ...(appearance === undefined ? {} : { appearance: appearance.value }), createdAt: now, updatedAt: now }
        : { ...existing, eventId: event.id, targetResolution: resolution.value, safeAreaMargin: margin.value, blackoutAppearance: blackout.value, ...(appearance === undefined ? {} : { appearance: appearance.value }), updatedAt: now }
