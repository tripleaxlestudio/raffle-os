...
export function ProductionHistoryPage() {
  ...
  return <section aria-labelledby="history-title" className="history-page">
    <PageHeader eyebrow="Official records" headingId="history-title" title="History" description={`Authoritative Live DrawSessions for ${workspace.event.name}.`} actions={hasHistory ? <ButtonLink variant="secondary" to="/draw/live">Open Draw Sessions</ButtonLink> : undefined} />
    {hasHistory ? <StatusBanner badge="Selected Event" title={workspace.event.name} tone="info">Only sessions owned by this Event are shown. History is read-only; decisions remain in Pending Results.</StatusBanner> : null}
    ...
    {filtered.length === 0 && !hasHistory ? <Card className="history-empty-state" padding="lg"><p className="operator-eyebrow">Official history</p><div className="history-empty-state__copy"><h2>No official draws yet</h2><p>This Event does not have any persisted Live draw results yet.</p><p>Completed official draws will appear here automatically.</p></div><ButtonLink size="lg" to="/draw/setup">Open Draw Setup</ButtonLink></Card> : filtered.length === 0 ? <Card padding="md"><h2>No official sessions</h2><p>No sessions match the selected filters.</p><ButtonLink to="/draw/setup">Open Draw Setup</ButtonLink></Card> : <div className="draw-session-queue" aria-label="Official history sessions">{filtered.map((item) => <HistorySessionCard expanded={selected?.session.id === item.session.id} item={item} key={item.session.id} />)}</div>}
