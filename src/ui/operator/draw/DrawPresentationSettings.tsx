  ...
  return <section className="draw-setup-section draw-presentation-settings" aria-labelledby="presentation-settings-title">
    ...
    {configuration.presentationMode === 'random-number-roll' ? <div className="presentation-roll-controls">
      <div className="presentation-roll-controls__row">
        <fieldset className="presentation-control-group"><legend>Roll control</legend><div className="presentation-segmented presentation-segmented--control" role="radiogroup" aria-label="Roll control"><SegmentedChoice label="Timed" selected={configuration.rollStopMode === 'timed'} disabled={disabled} onSelect={() => update('rollStopMode', 'timed')} /><SegmentedChoice label="Manual Stop" selected={configuration.rollStopMode === 'manual'} disabled={disabled} onSelect={() => update('rollStopMode', 'manual')} /></div></fieldset>
        {configuration.rollStopMode === 'timed' ? <fieldset className="presentation-control-group"><legend>Roll duration</legend><div className="presentation-segmented" role="radiogroup" aria-label="Roll duration">{ROLLING_DURATIONS_SECONDS.map((duration) => <SegmentedChoice key={duration} label={`${duration} sec`} selected={configuration.rollDurationSeconds === duration} disabled={disabled} onSelect={() => update('rollDurationSeconds', duration)} />)}</div></fieldset> : <div className="presentation-control-group presentation-manual-state"><span className="presentation-control-group__label">Manual control</span><strong>Stop &amp; Reveal from Live Draw</strong><small className="presentation-control-group__hint">Rolling continues until the operator stops it.</small></div>}
        <fieldset className="presentation-control-group"><legend>Roll speed</legend><div className="presentation-segmented presentation-segmented--speed" role="radiogroup" aria-label="Roll speed">{DRAW_ROLL_SPEED_PRESETS.map((preset) => <SegmentedChoice key={preset.label} label={preset.label} description={preset.description} selected={configuration.rollSpeedPerSecond === preset.value} disabled={disabled} onSelect={() => update('rollSpeedPerSecond', preset.value)} />)}</div></fieldset>
      </div>
      <fieldset className="presentation-control-group presentation-roll-controls__reveal"><legend>Reveal mode</legend><div className="presentation-segmented presentation-segmented--wide" role="radiogroup" aria-label="Reveal mode"><SegmentedChoice label="Reveal Together" selected={configuration.revealMode === DRAW_REVEAL_MODES[0]} disabled={disabled} onSelect={() => update('revealMode', 'all-together' satisfies DrawRevealMode)} /><SegmentedChoice label="Reveal Sequentially" selected={configuration.revealMode === DRAW_REVEAL_MODES[1]} disabled={disabled} onSelect={() => update('revealMode', 'sequential' satisfies DrawRevealMode)} /></div><small className="presentation-control-group__hint">For one winner, both reveal modes look the same. With multiple winners, choose whether tickets appear together or one at a time.</small></fieldset>
