KOCOKAN - Packaging Pilot P2 (Windows x64)

Keep this entire folder together. Run Kocokan.exe. Wait for Running, then
click Launch Kocokan. Node and .NET are included; no installation is needed.
Origin: http://127.0.0.1:47882/
Hide minimizes to taskbar. Reopen from taskbar or run Kocokan.exe again.
Quit (or X) asks for confirmation and stops the local server.
If port 47882 is occupied, resolve the conflicting application and click Retry.
Do not delete DLLs beside Kocokan.exe: this is a self-contained folder build.

Event data remains in the browser profile at the canonical origin. Moving this
folder does not export or transfer browser data. Use a consistent browser/profile.
Closing the browser does not stop the server. The draw controller remains in
the browser; the launcher does not run draws in the background.

Unsigned pilot. No installer, updater, LAN mode or cloud service included.
Third-party licenses: notices/ and runtime license files at this folder root.
