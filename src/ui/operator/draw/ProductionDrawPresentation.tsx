...
function WinnerRevealList({ result, mode }: { readonly result: PresentationResultProjection; readonly mode: 'live' | 'practice' }) {
  return <ol className={`production-winner-list production-winner-list--${result.winners.length}`} aria-label={`${result.winners.length} ${mode === 'live' ? 'official' : 'Practice'} winners`}>{result.winners.map((winner) => <li key={winner.winnerId}><span>#{winner.sequence}</span><strong>{winner.ticketNumber}</strong></li>)}</ol>
}
...
function RedrawHistoryPanel({ lineage }: { readonly lineage?: readonly PresentationLineageEntry[] }) {
  ...
  return <section aria-label="Redraw history" className="production-redraw-history-panel"><p className="production-redraw-history-panel__title">Redraw history · {replaced.length} replaced winner{replaced.length === 1 ? '' : 's'}</p><details className="production-redraw-history-panel__details"><summary>View history</summary><div className="production-winner-lineage__history-list"><ul aria-label="Previous replaced tickets">{replaced.map((entry) => <li key={entry.winnerId}><strong>{entry.ticketNumber}</strong><span>REPLACED · ABSENT</span></li>)}</ul></div></details></section>
}
...
export function ProductionDrawPresentation({ result, activeResult, mode, eventName, eventId = 'production-event', displayConfigurationId = eventId, prizeCategory, prizeName, checkpoints, practiceResult, onFailure, initialPresentation, onHandoff, sharedPublisher, onResetPractice, resetPending = false, presentationPolicy = PRESENTATION_POLICY, presentationConfiguration, audienceStatus, recap, backTo, onRecoveryBack, onReviewPendingResults, reviewPendingResultsAvailable = false, onQuickRedraw, redrawLineage, displayConfiguration }: ProductionDrawPresentationProps) {
  ...
  const blackoutControl = blackoutRequested ? <div className="presentation-blackout-control"><p>Audience publication is currently blacked out.</p><Button icon={<Icon name="X" />} variant="secondary" onClick={() => { void controller.setBlackout(false) }}>End blackout</Button></div> : null
  ...
  async function confirmQuickRedraw() {
    ...
    try { await onQuickRedraw(selectedRedrawWinners.map((winner) => winner.winnerId)); setQuickRedrawOpen(false); setSelectedRedrawWinnerIds([]) } catch (cause: unknown) { setQuickRedrawError(cause instanceof Error ? cause.message : 'The redraw could not be completed safely.') }
  ...
  const quickRedrawDialog = quickRedrawAvailable ? <Modal closeOnEscape={!quickRedrawBusy} eyebrow="REDRAW CONFIRMATION" footer={<><Button icon={<Icon name="ArrowLeft" />} disabled={quickRedrawBusy} onClick={() => { setQuickRedrawError(null); setQuickRedrawOpen(false) }} variant="quiet">Back</Button><Button className="production-quick-redraw-dialog__review" disabled={quickRedrawBusy} icon={<Icon name="ClipboardCheck" />} onClick={() => { setQuickRedrawError(null); setQuickRedrawOpen(false); onReviewPendingResults?.() }} variant="secondary">Review Instead</Button><Button className="production-quick-redraw-dialog__confirm" disabled={quickRedrawBusy} icon={<Icon name="RotateCcw" />} isLoading={quickRedrawBusy} onClick={() => { void confirmQuickRedraw() }} variant="danger">Confirm Redraw</Button></>} onClose={() => { if (!quickRedrawBusy) { setQuickRedrawError(null); setQuickRedrawOpen(false) } }} open={quickRedrawOpen} showCloseButton={false} title="Redraw winner">
    <div className="production-quick-redraw-dialog__content">
      <section aria-labelledby="quick-redraw-current-winner" className="production-quick-redraw-dialog__winner">
        <p id="quick-redraw-current-winner" className="production-quick-redraw-dialog__label">Current winner</p>
        <strong className="production-quick-redraw-dialog__ticket">{quickRedrawTickets.length === 1 ? `Ticket ${quickRedrawTicket}` : `${quickRedrawTickets.length} winners selected`}</strong>
      </section>
      <aside className="production-quick-redraw-dialog__warning">
        <strong>This winner will be replaced</strong>
        <p>{quickRedrawTickets.length === 1 ? `Ticket ${quickRedrawTicket}` : `Tickets ${quickRedrawTickets.join(', ')}`} will be marked as Absent and excluded from this replacement draw.</p>
        <p>The original result will remain in official history for audit purposes.</p>
      </aside>
      <section aria-labelledby="quick-redraw-summary" className="production-quick-redraw-dialog__summary">
        <p id="quick-redraw-summary" className="production-quick-redraw-dialog__label">Replacement draw</p>
        <dl>
          <div><dt>Prize</dt><dd>{prizeName}</dd></div>
          <div><dt>Category</dt><dd>{prizeCategory}</dd></div>
          <div><dt>New winner{quickRedrawTickets.length === 1 ? '' : 's'}</dt><dd>{quickRedrawTickets.length}</dd></div>
        </dl>
      </section>
      <p className="production-quick-redraw-dialog__supporting-copy">{quickRedrawTickets.length === 1 ? 'A new eligible participant will be selected using the existing Live draw rules.' : 'New eligible participants will be selected using the existing Live draw rules.'}</p>
      {quickRedrawError === null ? null : <p role="alert">{quickRedrawError}</p>}
    </div>
  </Modal> : null
  const redrawSelectionDialog = quickRedrawAvailable && redrawableWinners.length > 1 ? <Modal closeOnEscape={!quickRedrawBusy} description="Choose the winner(s) who are unavailable and should be replaced." eyebrow="SELECT WINNERS" footer={<><Button icon={<Icon name="CircleX" />} disabled={quickRedrawBusy} onClick={() => { setRedrawSelectionOpen(false); setSelectedRedrawWinnerIds([]) }} variant="quiet">Cancel</Button><Button icon={<Icon name="RotateCcw" />} disabled={quickRedrawBusy || selectedRedrawWinnerIds.length === 0} onClick={() => { setRedrawSelectionOpen(false); setQuickRedrawError(null); setQuickRedrawOpen(true) }} variant="danger">Continue to Redraw</Button></>} onClose={() => { if (!quickRedrawBusy) { setRedrawSelectionOpen(false); setSelectedRedrawWinnerIds([]) } }} open={redrawSelectionOpen} showCloseButton={false} title="Select winners to redraw"><div className="production-redraw-selection-dialog"><div className="production-redraw-selection-dialog__toolbar"><p aria-live="polite"><strong>{selectedRedrawWinnerIds.length}</strong> of {redrawableWinners.length} selected</p><div><Button disabled={quickRedrawBusy || selectedRedrawWinnerIds.length === redrawableWinners.length} onClick={() => setSelectedRedrawWinnerIds(redrawableWinners.map((winner) => winner.winnerId))} size="sm" variant="quiet">Select all</Button><Button disabled={quickRedrawBusy || selectedRedrawWinnerIds.length === 0} onClick={() => setSelectedRedrawWinnerIds([])} size="sm" variant="quiet">Clear</Button></div></div><fieldset className="production-redraw-selection-dialog__list"><legend className="sr-only">Current pending winners</legend>{redrawableWinners.map((winner) => { const selected = selectedRedrawWinnerIds.includes(winner.winnerId); return <label key={winner.winnerId} className={`production-redraw-selection-dialog__option${selected ? ' production-redraw-selection-dialog__option--selected' : ''}`}><input aria-label={`Select winner #${winner.sequence}, ticket ${winner.ticketNumber}`} checked={selected} disabled={quickRedrawBusy} onChange={() => setSelectedRedrawWinnerIds((current) => current.includes(winner.winnerId) ? current.filter((id) => id !== winner.winnerId) : [...current, winner.winnerId])} type="checkbox" /><span className="production-redraw-selection-dialog__position">#{winner.sequence}</span><strong className="production-redraw-selection-dialog__ticket">{winner.ticketNumber}</strong><span className="production-redraw-selection-dialog__state">{selected ? 'Selected' : 'Available'}</span></label> })}</fieldset></div></Modal> : null
...
export function PresentationSupport({ recap, previewStage, audienceStatus, eventName, prizeCategory, prizeName, result, blackoutRequested, publicSnapshot, displayConfiguration }: { readonly recap?: ProductionDrawRecap; readonly previewStage: 'ready' | 'countdown' | 'rolling' | 'reveal' | 'pending-handoff'; readonly audienceStatus?: { readonly label: string; readonly detail: string; readonly displayUrl: string | null }; readonly eventName: string; readonly prizeCategory: string; readonly prizeName: string; readonly result?: PresentationResultProjection; readonly blackoutRequested: boolean; readonly publicSnapshot?: PublicDisplaySnapshot; readonly displayConfiguration?: Pick<DisplayConfiguration, 'safeAreaMargin' | 'blackoutAppearance' | 'targetResolution'> }) {
  const previewLabel = previewStage === 'ready' ? 'Next draw' : previewStage === 'pending-handoff' ? 'Completed' : previewStage
  void prizeCategory
  void prizeName
  ...
  return <aside className="production-presentation-support"><Card padding="md"><div className="production-support-section"><div className="production-support-section__heading"><span>Audience preview</span><Badge variant={blackoutRequested ? 'danger' : previewStage === 'ready' || previewStage === 'countdown' || previewStage === 'rolling' ? 'info' : 'success'}>{blackoutRequested ? 'Blackout' : previewLabel}</Badge></div><div className="production-preview" aria-label={`Public Audience preview for ${eventName}`} data-testid="production-preview" data-public-stage={blackoutRequested ? 'blackout' : previewStage}><div className="production-preview__viewport"><AudiencePresentation snapshot={snapshot} displayConfiguration={displayConfiguration} preview /></div></div></div><div className="production-support-section"><div className="production-support-section__heading"><span>Runtime status</span></div><small className="production-support-section__detail">Audience acknowledgement: {audienceStatus?.detail ?? 'No publisher status.'}</small><dl className="production-recap"><div><dt>Winner count</dt><dd>{recap?.winnerCount ?? result?.winners.length ?? '—'}</dd></div><div><dt>Eligible pool</dt><dd>{recap?.eligibleCount ?? '—'}</dd></div><div><dt>Winning rule</dt><dd>{recap?.winningRule ?? '—'}</dd></div><div><dt>Presentation</dt><dd>{recap === undefined ? '—' : `${recap.countdownSeconds}s countdown · ${recap.rollingSeconds}s rolling`}</dd></div></dl></div></Card></aside>
}
