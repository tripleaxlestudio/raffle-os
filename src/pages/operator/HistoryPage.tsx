...
export function PrototypeHistoryPage() {
  ...
  if (production) return <ProductionHistoryPage />
    ...
    void Promise.resolve().then(async () => {
      try {
        ...
        for (const event of events) {
          const eventSessions = await services.sessions.findByEventId(event.id)
          for (const session of eventSessions.filter((candidate) => candidate.mode === 'live')) {
...
function statusLabel(status: 'pending' | 'confirmed' | 'cancelled'): string { return status === 'pending' ? 'Pending' : status === 'confirmed' ? 'Confirmed' : 'Cancelled' }
