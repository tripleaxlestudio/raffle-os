...
function DashboardOperations({ audienceStatus, displayUrl, drawState, pendingCount }: { readonly audienceStatus: ReturnType<typeof presentAudienceConnection>; readonly displayUrl: string | null; readonly drawState: { status: 'loading' } | { status: 'error' } | { status: 'ready'; item: DrawSessionQueueItem | null }; readonly pendingCount: number }) {
  ...
  return <section aria-label="Operasi Dasbor" className="production-dashboard__operations">
    ...
    <Card className="production-dashboard__operations-panel" padding="md"><div className="production-dashboard__panel-heading"><div><p className="operator-eyebrow">OPERASI</p></div></div><dl className="production-dashboard__operation-list"><div><dt>Tampilan Audiens</dt><dd><span className={`production-dashboard__status-dot production-dashboard__status-dot--${audienceStatus.tone}`} />{audienceStatus.label}</dd></div><div><dt>Menunggu Konfirmasi</dt><dd>{pendingCount}</dd></div></dl><div className="production-dashboard__operation-actions">{displayUrl === null ? <ButtonLink icon={<Icon name="MonitorCog" />} variant="secondary" to="/settings">Buka Pengaturan Tampilan</ButtonLink> : <AudienceDisplayButton displayUrl={displayUrl} variant="secondary" />}{pendingCount > 0 ? <ButtonLink icon={<Icon name="ClipboardCheck" />} variant="secondary" to="/draw/pending">Buka Hasil</ButtonLink> : null}</div></Card>
  </section>
}
