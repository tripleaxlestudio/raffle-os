...
export function DrawSessionQueuePage() {
  ...
  return <section aria-labelledby="draw-queue-title" className="draw-live-queue">
    <PageHeader eyebrow="LIVE OPERATIONS" headingId="draw-queue-title" title="Live Draw" description="Select a ready DrawSession to operate." actions={<ButtonLink size="lg" to="/draw/setup">Open Draw Setup</ButtonLink>} />
    <StatusBanner className="draw-live-queue__status" badge={actionRequired > 0 ? 'Action required' : 'Queue ready'} title={actionRequired > 0 ? `${actionRequired} session${actionRequired === 1 ? '' : 's'} need attention` : 'Draw control deck ready'} tone={actionRequired > 0 ? 'warning' : 'info'}>Practice is rehearsal-only. Live results remain pending until explicitly confirmed.</StatusBanner>
