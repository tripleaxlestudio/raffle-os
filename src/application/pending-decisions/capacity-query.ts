...
export function calculateReplacementCapacity(input: ReplacementCapacityQuery): ReplacementCapacityResult {
  ...
  const targets = new Set(input.targetWinnerIds)
  const occupiedTickets = new Set(input.winners.filter((winner) => winner.status !== 'cancelled' && !targets.has(winner.id)).map((winner) => winner.ticketNumber))
