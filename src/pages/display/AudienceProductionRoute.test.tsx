...
function expectFullViewportAudienceStage() {
  ...
  const visibleChildren = Array.from(page?.children ?? []).filter((child) => !child.classList.contains('sr-only'))
  expect(visibleChildren.some((child) => child.classList.contains('audience-stage'))).toBe(true)
  expect(visibleChildren.some((child) => child.classList.contains('audience-presentation-footer'))).toBe(true)
  expect(visibleChildren.filter((child) => !child.matches('.audience-display-logo, .audience-stage, .audience-presentation-footer'))).toHaveLength(0)
...
describe('production Audience route', () => {
  ...
  it('renders valid public snapshots without operator controls', () => {
    ...
    act(() => { publisher.publish(stateMessage(2, { stage: 'countdown', stageStartedAt: '2026-08-05T00:00:00.000Z', countdownValue: 2, eventName: '24th K-Link Indonesia Anniversary', prizeCategory: 'Voucher', prizeName: 'Voucher Indomaret Rp50.000' })) })
    expect(screen.getByRole('heading', { name: 'Bersiap' })).toBeVisible()
    expect(screen.getByLabelText('Hitung mundur: 2')).toBeVisible()
    expect(screen.getByRole('heading', { name: 'Voucher Indomaret Rp50.000' })).toBeVisible()
    expect(screen.getByText('Voucher')).toBeVisible()
    const countdownStage = document.querySelector('.countdown-stage')
    const countdownHeader = countdownStage?.querySelector('.audience-draw-header')
    const countdownContent = countdownStage?.querySelector('.countdown-stage__content')
    expectCenteredDrawHeader()
    expect(countdownHeader?.nextElementSibling).toBe(countdownContent)
    expectFullViewportAudienceStage()
    ...
    expect(document.querySelector('.audience-stage')).toHaveAttribute('data-audience-state', 'confirmed')
    expect(screen.getAllByText('Dikonfirmasi')).toHaveLength(2)
  ...
  it('updates only the authoritative confirmed tile in place during partial verification', () => {
    ...
    expect(after.map((tile) => tile.textContent?.replace('Dikonfirmasi', '').replace('Pemenang', '').trim())).toEqual(['00073', '00052', '00059'])
    expect(after[0]).toHaveTextContent('Dikonfirmasi')
    expect(after.slice(1).every((tile) => tile.textContent?.includes('Pemenang'))).toBe(true)
    controller.close()
    publisher.close()
  })
