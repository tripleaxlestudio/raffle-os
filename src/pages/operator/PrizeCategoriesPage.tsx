...
export function PrizeCategoriesPage() {
  ...
  return <section className={ui('prize-categories-page')} aria-labelledby="categories-title"><div className={ui('page-header')}><div className={ui('page-header__copy')}><p className={ui('page-header__eyebrow')}>Pengaturan produksi</p><h1 id="categories-title">Kategori Hadiah</h1><p className={ui('page-header__description')}>Kelola kategori hadiah untuk {selectedEvent.name}.</p><p className={ui('prize-categories-page__info')}>Perubahan hadiah berlaku untuk undian berikutnya. Record undian yang sudah ada tidak berubah.</p></div></div>
    ...
    <div className={ui('prize-categories-page__layout')}>
      <Card padding="md" tone="raised" className={ui('prize-categories-page__form-card')}><div className={ui('prize-categories-page__card-heading')}><div><p className={ui('prize-categories-page__eyebrow')}>{editing === null ? 'Kategori hadiah baru' : 'Edit record'}</p><h2>{editing === null ? 'Buat Kategori Hadiah' : 'Edit Kategori Hadiah'}</h2></div></div>
        <form className={ui('prize-categories-page__form')} onSubmit={(event) => { event.preventDefault(); void submit() }}>
          <fieldset className={ui('preparation__group')}><legend>Identitas hadiah</legend>
          ...
          <div className={ui('prize-categories-page__two-column')}><Input label="Nama hadiah" required maxLength={160} disabled={isReadOnly} value={form.prizeName} placeholder="contoh: Sepeda" onChange={(event) => setForm({ ...form, prizeName: event.target.value })} /><Input label="Nama sponsor" maxLength={120} disabled={isReadOnly} value={form.sponsorName} placeholder="Opsional" onChange={(event) => setForm({ ...form, sponsorName: event.target.value })} /></div>
          </fieldset><fieldset className={ui('preparation__group')}><legend>Detail tambahan</legend>
          <label className={ui('ui-field')}><span className={ui('ui-field__label')}>Deskripsi</span><textarea className={ui('ui-input')} maxLength={500} disabled={isReadOnly} value={form.description} placeholder="Keterangan opsional tentang hadiah ini" onChange={(event) => setForm({ ...form, description: event.target.value })} /></label>
          ...
          <div className={ui('prize-categories-page__form-actions')}><Button icon={<Icon name={editing === null ? 'Plus' : 'Save'} />} type="submit" disabled={!isFormValid || isReadOnly} isLoading={submitting}>{editing === null ? 'Buat Kategori Hadiah' : 'Simpan perubahan'}</Button>{editing !== null ? <Button icon={<Icon name="X" />} type="button" variant="secondary" onClick={reset}>Batal</Button> : null}</div>
        </form>
      </Card>
      <Card padding="md" tone="raised" className={ui('prize-categories-page__list-card')}><div className={ui('prize-categories-page__card-heading')}><div><p className={ui('prize-categories-page__eyebrow')}>{items.length === 0 ? 'Pengaturan hadiah' : 'Hadiah yang sudah diatur'}</p><h2>Kategori Hadiah</h2></div><span className={ui('prize-categories-page__record-count')}>{items.length} kategori</span></div>
        {loading ? <p role="status">Memuat kategori hadiah…</p> : items.length === 0 ? <div className={ui('prize-categories-page__empty')} role="status"><strong>Belum ada kategori hadiah</strong><p>Buat kategori pertama sebelum mengatur undian.</p></div> : <ul className={ui('prize-categories-page__list')}>{items.map((category) => <li className={ui('prize-categories-page__category-card')} key={category.id}><div className={ui('prize-categories-page__category-main')}><div className={ui('prize-categories-page__category-heading')}><div><h3>{category.name}</h3><p>{category.prizeName}</p></div><span className={ui('prize-categories-page__order')}>Urutan {category.displayOrder}</span></div>{category.sponsorName ? <p className={ui('prize-categories-page__sponsor')}>Disponsori oleh {category.sponsorName}</p> : null}{category.description ? <p className={ui('prize-categories-page__description')}>{category.description}</p> : null}</div><div className={ui('prize-categories-page__category-actions')}><Button icon={<Icon name="Pencil" />} size="sm" variant="quiet" disabled={isReadOnly} onClick={() => beginEdit(category)}>Edit</Button></div></li>)}</ul>}
      </Card>
    </div>
  </section>
}
