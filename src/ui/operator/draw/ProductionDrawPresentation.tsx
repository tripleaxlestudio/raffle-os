...
export function ProductionDrawRunHeader({ mode, stage, eventName, prizeCategory, prizeName, recap, audienceStatus, backTo }: { readonly mode: 'live' | 'practice'; readonly stage: string; readonly eventName: string; readonly prizeCategory: string; readonly prizeName: string; readonly recap?: ProductionDrawRecap; readonly audienceStatus?: { readonly label: string; readonly detail: string; readonly displayUrl: string | null }; readonly backTo?: string }) {
  return <header className="production-draw-run-header"><div className="production-draw-run-header__identity"><div className="production-draw-run-header__title"><Badge variant={mode === 'practice' ? 'warning' : 'danger'}>{mode === 'practice' ? 'PRACTICE' : 'LIVE'}</Badge><div><p className="operator-eyebrow">{mode === 'practice' ? 'Rehearsal only' : 'Official result locked'}</p><h1 id="presentation-heading">{stage}</h1><p>{eventName} · {prizeCategory} · {prizeName}</p></div></div><dl className="production-draw-run-header__metrics"><div><dt>Winners</dt><dd>{recap?.winnerCount ?? '—'}</dd></div><div><dt>Eligible</dt><dd>{recap?.eligibleCount ?? '—'}</dd></div><div><dt>Presentation</dt><dd>{recap === undefined ? '—' : `${recap.countdownSeconds}s countdown · ${recap.rollingSeconds}s rolling`}</dd></div></dl></div><div className="production-draw-run-header__audience"><span className="operator-eyebrow">Audience Display</span><Badge variant={audienceStatus?.label === 'Connected' ? 'success' : audienceStatus?.label === 'Waiting' ? 'warning' : 'danger'}>{audienceStatus?.label ?? 'Unavailable'}</Badge><small>{audienceStatus?.detail ?? 'No production publisher status.'}</small><div className="production-draw-run-header__actions">{backTo === undefined ? null : <ButtonLink to={backTo} variant="quiet">Back to Live Draw</ButtonLink>}{audienceStatus?.displayUrl === null || audienceStatus === undefined ? null : <ButtonLink to={audienceStatus.displayUrl} target="_blank" rel="noreferrer" variant="quiet">Open Audience Display</ButtonLink>}</div></div></header>
}
