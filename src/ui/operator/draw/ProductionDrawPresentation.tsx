...
export function ProductionDrawRunHeader({ mode, stage, eventName, prizeCategory, prizeName, recap, audienceStatus, backTo }: { readonly mode: 'live' | 'practice'; readonly stage: string; readonly eventName: string; readonly prizeCategory: string; readonly prizeName: string; readonly recap?: ProductionDrawRecap; readonly audienceStatus?: { readonly label: string; readonly detail: string; readonly displayUrl: string | null }; readonly backTo?: string }) {
  return <header className="production-draw-run-header"><div className="production-draw-run-header__identity"><div className="production-draw-run-header__title"><Badge variant={mode === 'practice' ? 'warning' : 'danger'}>{mode === 'practice' ? 'PRACTICE' : 'LIVE'}</Badge><div><p className="operator-eyebrow">{mode === 'practice' ? 'Rehearsal only' : 'Official result locked'}</p><h1 id="presentation-heading">{stage}</h1><p>{eventName} · {prizeCategory} · {prizeName}</p></div></div><dl className="production-draw-run-header__metrics"><div><dt>Winners</dt><dd>{recap?.winnerCount ?? '—'}</dd></div><div><dt>Eligible</dt><dd>{recap?.eligibleCount ?? '—'}</dd></div><div><dt>Presentation</dt><dd>{recap === undefined ? <span>—</span> : <><span>{recap.countdownSeconds}s countdown</span><span>{recap.rollingSeconds}s rolling</span></>}</dd></div></dl></div><div className="production-draw-run-header__audience"><div className="production-draw-run-header__audience-status"><span className="operator-eyebrow">Audience Display</span><div className="production-draw-run-header__audience-state"><Badge variant={audienceStatus?.label === 'Connected' ? 'success' : audienceStatus?.label === 'Waiting' ? 'warning' : 'danger'}>{audienceStatus?.label ?? 'Unavailable'}</Badge></div><small>{audienceStatus?.detail ?? 'No production publisher status.'}</small></div><div className="production-draw-run-header__actions">{backTo === undefined ? null : <ButtonLink to={backTo} size="sm" variant="secondary" className="production-draw-run-header__back">Back to Live Draw</ButtonLink>}{audienceStatus?.displayUrl === null || audienceStatus === undefined ? null : <ButtonLink to={audienceStatus.displayUrl} target="_blank" rel="noreferrer" size="sm" variant="secondary">Open Audience Display</ButtonLink>}</div></div></header>
}
...
export function ProductionDrawPresentation({ result, mode, eventName, eventId = 'production-event', displayConfigurationId = eventId, prizeCategory, prizeName, checkpoints, practiceResult, onFailure, initialPresentation, onHandoff, sharedPublisher, onResetPractice, resetPending = false, presentationPolicy = PRESENTATION_POLICY, audienceStatus, recap, backTo }: ProductionDrawPresentationProps) {
